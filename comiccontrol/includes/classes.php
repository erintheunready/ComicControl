	...
	public function displayTitle(){
		...
		if($this->moduletype == "comic" || ($this->moduletype == "blog" && $this->subslug != "")){
			...
			if($this->subslug == "archive"){
				...
				echo $lang['search'] . ' - ' . urldecode($this->searchterm);
			}
				...
				echo $lang['userpage'] . ' ' . $this->pagenum;
			}
...
class CC_Module{
	...
	public function getPageNav($numpages){
		...
		if($page > 1){
			echo '<a href="' . $ccsite->root . $ccsite->relativeroot . $this->slug . '/' . $pagedir . '/' . ($page-1) . '">' . $lang['blogprev'] . '</a>';
		}
		if($page < $numpages){
			echo '<a href="' . $ccsite->root . $ccsite->relativeroot . $this->slug . '/' . $pagedir . '/' . ($page+1) . '">' . $lang['blognext'] . '</a>';
		}
		echo '</div>';
		...
		$ellipsis1 = false;
		$ellipsis2 = false;
		echo '<div class="cc-pagelist">' . $lang["Page"] . ' ';
...
class CC_Text extends CC_Module{
	...
	public function __construct($moduleinfo){
		...
		//if page wasn't found, deliver an error message
		else{
			echo '<div class="cc-errormsg">' . $lang['modulenotfound'] . '</div>';
		}
...
class CC_Comic extends CC_Module{
		...
		if($comic['title'] != ""){
			...
			//handle case for everything else and display comic
				...
				if($comic != $this->getSeq("last") && $comic['altnext'] == ""){
					...
					echo '<a href="' . $ccsite->root . $ccsite->relativepath . $this->slug . '/' . $nextcomic['slug'] . '">';
				}
				...
				if($this->options['contentwarnings'] == "on" && trim($comic['contentwarning']) != ""){
					echo '<script>var contentwarningtext = "' . str_replace('"','&quot;',$comic['contentwarning']) . $lang['<br />Click to view this page'] . '";</script>';
					?>
					<script>
  ...
  inputClass: Hammer.SUPPORT_POINTER_EVENTS ? Hammer.PointerEventInput : Hammer.TouchInput, touchAction : 'auto'
			...
			//include appropriate javascript for swiping through comic on mobile
			if($this->options['swipeaction'] == "navigate"){
	  ...
	  inputClass: Hammer.SUPPORT_POINTER_EVENTS ? Hammer.PointerEventInput : Hammer.TouchInput, touchAction : 'auto'
					...
					<? if(is_array($prevcomic)){ ?>
					swipeNav.on("swiperight",function(){
						$('body').append('<div class="cc-fullscreen-overlay"><div class="cc-fullscreen-center"><img src="<?=$ccsite->root . $ccsite->relativepath . $ccsite->ccroot?>images/leftarrow.png"></div></div>');
						$('body, html').css({'height':'100%','overflow':'hidden'});
						$('.cc-fullscreen-overlay').fadeIn('fast', function(){
						window.location.href = '<?=$ccsite->root . $ccsite->relativepath . $this->slug . '/' . $prevcomic['slug']?>'; 
						});
					});
					<? }
					if(is_array($nextcomic)){ ?>
					swipeNav.on("swipeleft",function(){
						$('body').append('<div class="cc-fullscreen-overlay"><div class="cc-fullscreen-center"><img src="<?=$ccsite->root . $ccsite->relativepath . $ccsite->ccroot?>images/rightarrow.png"></div></div>');
						$('body, html').css({'height':'100%','overflow':'hidden'});
						$('.cc-fullscreen-overlay').fadeIn('fast', function(){
							window.location.href = '<?=$ccsite->root . $ccsite->relativepath . $this->slug . '/' . $nextcomic['slug']?>'; 
						});
					});
					<? } ?>
				</script>
		...
		//if current comic wasn't found, deliver error message
			...
			echo '<div class="cc-errormsg">' . $lang['There is no comic with this ID.'] . '</div>';	
		}
	...
	public function navDisplay(){
		...
		if($currentcomic['title'] != ""){
			...
			//if it's not, generate the first and previous comic buttons
				...
				$firstbutton = '<a class="cc-first" rel="first" href="' . $ccsite->root . $ccsite->relativepath . $this->slug . '/' . $firstcomic['slug'] . '">' . $this->options['firsttext'] . '</a>';
			
				$prevcomic = $this->getSeq("prev");
				$prevbutton = '<a class="cc-prev" rel="prev" href="' . $ccsite->root . $ccsite->relativepath . $this->slug . '/' . $prevcomic['slug'] . '">' . $this->options['prevtext'] . '</a>';
			}
			...
			//if it's not, generate the last and next comic buttons
				...
				$lastbutton = '<a class="cc-last" rel="last" href="' . $ccsite->root . $ccsite->relativepath . $this->slug . '/' . $lastcomic['slug'] . '">' . $this->options['lasttext'] . '</a>';
			
				$nextcomic = $this->getSeq("next");
				$nextbutton = '<a class="cc-next" rel="next" href="' . $ccsite->root . $ccsite->relativepath . $this->slug . '/' . $nextcomic['slug'] . '">' . $this->options['nexttext'] . '</a>';
			}
	...
	public function displayNews(){
		...
		if($news['title'] != ""){
			...
			echo '</div>';
			echo '<div class="cc-publishtime">' . str_replace('%t',date($ccsite->timeformat,$news['publishtime']),str_replace('%d',date($ccsite->dateformat,$news['publishtime']),$lang['Posted %d at %t'])) . '</div>';
		...
		//if the comic wasn't found, deliver an error message
			...
			echo '<div class="cc-errormsg">' . $lang['There is no news post with this ID.'] . '</div>';
		}
	...
	public function displayTranscript(){
		...
		if($comic['transcript'] != ""){
			...
			if($this->options['transcriptclick'] == "on") echo '<div id="cc-transcriptclick"><button type="button" id="cc-transcriptbutton">' . $lang['clicktranscripton'] . '</button>';
	...
	public function displayComments(){
		...
		if($comic['title'] != ""){
			...
			if($ccpage->isindex){
				echo '<div class="cc-commentlink"><a href="' . $ccsite->root . $ccsite->relativepath . $this->slug . '/' . $comic['slug'] . '#disqus_thread" data-disqus-identifier="' . $comic['commentid'] . '">' . $lang['userviewpostcomments'] . '</a></div>';
				?>
				<script id="dsq-count-scr" src="//<?=$ccsite->commentname?>.disqus.com/count.js" async></script>
				<?
			}
			
			//if it's not the index, display the comment thread
			else{
				echo '<div class="cc-commentheader">' . $lang['usercomments'] . '</div><div class="cc-commentbody">';
				?>
			...
			if(!($dropdown)){
				...
				if(!empty($firstpage)){
					...
					if($this->options['pagethumbs'] == "off" && $this->options['chapterthumbs'] == "on"){
						echo '<div class="cc-storyline-thumb"><a href="' . $ccsite->root . $ccsite->relativepath . $this->slug . '/' . $firstpage['slug'] . '"><img src="' . $ccsite->root . $ccsite->relativepath . 'comicsthumbs/' . $firstpage['comicthumb'] . '" /></a></div>';
					}
	...
	public function search(){
		...
		//display the results
		echo '<div class="cc-searchheader">' . $lang['comicstagged'] . '"' . urldecode($ccpage->searchterm) . '" - ' . $lang['userpage'] . ' ' . $page . '</div>';
		...
		//deliver error message if no results found
			...
			echo '<div class="cc-errormsg">' . $lang['nocomicsfound'] . '</div>';
...
class CC_Gallery extends CC_Module{
	...
	public function display(){
		...
		//deliver error message if no images found
			...
			echo '<div class="cc-errormsg">' . $lang['therearenoimages'] . '</div>';
		}
		echo '</div>';
...
class CC_Blog extends CC_Module{
	...
	public function display(){
		...
		if($this->browsing){
			...
			if(is_array($posts[0])){
				if($ccpage->subslug == "search") echo '<div class="cc-searchheader">' . $lang['blogstagged'] . '"' . urldecode($ccpage->searchterm) . '" - ' . $lang['userpage'] . ' ' . $page . '</div>';
			...
			//deliver error message if no posts found
				...
				if($ccpage->subslug == "search") echo '<div class="cc-errormsg">' . $lang["No posts were found with this tag."] . '</div>';
				else echo $lang["There are no posts in this blog."];
			}
	...
	public function recentPosts($num){
		...
		echo '<div style="clear:both;"></div><div class="cc-readmore"><a href="' . $ccsite->root . $ccsite->relativepath . $this->slug . '">' . $lang['View more posts...'] . '</a></div>';
	}
	...
	public function displaySinglePost($slug){
		...
		if($post['title'] != ""){
			...
			echo '<div class="cc-blog-publishtime">' . str_replace('%t',date($ccsite->timeformat,$post['publishtime']),str_replace('%d',date($ccsite->dateformat,$post['publishtime']),$lang['Posted %d at %t'])) . '</div>';
		...
		//deliver error message if the post wasn't found
			...
			echo '<div class="cc-errormsg">' . $lang['thereisnopost'] . '</div>';
		}
	...
	public function displayComments($post){
		...
		if($this->browsing){
			echo '<div class="cc-commentlink"><a href="' . $ccsite->root . $ccsite->relativepath . $this->slug . '/' . $post['slug'] . '#disqus_thread" data-disqus-identifier="' . $post['commentid'] . '">' . $lang['userviewpostcomments'] . '</a></div>';
			?>
			<script id="dsq-count-scr" src="//<?=$ccsite->commentname?>.disqus.com/count.js" async></script>
			<?
		}
